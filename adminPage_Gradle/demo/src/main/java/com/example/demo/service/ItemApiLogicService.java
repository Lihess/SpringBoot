...
public class ItemApiLogicService extends BaseService<ItemApiRequest, ItemApiResponse, Item> {
    ...
    public Header<ItemApiResponse> create(Header<ItemApiRequest> request) {
        ...
        Item item = Item.builder().name(body.getName()).status(body.getStatus()).title(body.getTitle())
    ...
    public Header<ItemApiResponse> read(Long id) {
        ...
        return optional.map(item -> response(item)).orElseGet(() -> Header.ERROR("데이터 없음"));
    }
    ...
    public Header<ItemApiResponse> update(Header<ItemApiRequest> request) {
        ...
        return optional.map(item -> {
            ...
            return item;
        }).map(updateItem -> baseRepository.save(updateItem)).map(item -> response(item))
    ...
    public Header<ItemApiResponse> response(Item item) {
        ItemApiResponse itemApiResponse = ItemApiResponse.builder().id(item.getId()).status(item.getStatus())
                .name(item.getName()).title(item.getTitle()).content(item.getContent()).price(item.getPrice())
                .brandName(item.getBrandName()).registeredAt(item.getRegisteredAt())
